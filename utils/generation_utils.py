...
class GenerationMixin:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        if model_kwargs.get("model_list", None) is not None:
            model_list = model_kwargs["model_list"]
            kwargs_list = [copy.copy(model_kwargs) for i in range(len(model_list))]
        ...
        while True:
            ...
            if model_kwargs.get("model_list", None) is None:
            ...
            # forward pass to get next token
                ...
                **model_inputs,
                ...
                outputs = []
                log_probs = []
                for i, model in enumerate(model_kwargs["model_list"]):
                    model_inputs = self.prepare_inputs_for_generation(input_ids, **kwargs_list[i])
                    new_model_inputs = model_inputs.copy()
                    new_model_inputs["encoder_outputs"] = model_kwargs["encoder_outputs_list"][i]
                    output = model.model(**new_model_inputs,
                                         return_dict=True,
                                         output_attentions=output_attentions,
                                         output_hidden_states=output_hidden_states, )
                    ...
                    next_tokens_score = logits_processor(input_ids, next_token_logits)
                    log_probs.append(next_tokens_score)
                
                if model_kwargs.get("model_weight") is not None:
                    '''exp_probs = torch.exp(torch.stack(log_probs, dim=1))
                    weight = model_kwargs["model_weight"].unsqueeze(-1)
                    next_tokens_scores = torch.log(torch.sum(exp_probs * weight, dim = 1))'''
                    '''next_tokens_scores = torch.sum(torch.softmax(torch.stack(log_probs, dim=1), dim=-1) *
                                               model_kwargs["model_weight"].unsqueeze(-1), dim=1)'''
                    stack_probs = torch.stack(log_probs, dim=1)
                    next_tokens_scores = torch.sum(stack_probs * model_kwargs["model_weight"].unsqueeze(-1), dim = 1)
                else:
                    '''next_tokens_scores = torch.logsumexp(torch.stack(log_probs, dim=0), dim=0) - \
                                     math.log(len(model_kwargs["model_list"]))'''
                    stack_probs = torch.stack(log_probs, dim=0)
                    next_tokens_scores = torch.mean(stack_probs, dim=0)
            ...
            if model_kwargs.get("model_list") is None:
                ...
                for i in range(len(model_list)):
                    kwargs_list[i] = self._update_model_kwargs_for_generation(
                        outputs[i], kwargs_list[i], is_encoder_decoder=self.config.is_encoder_decoder
                    )
            cur_len = cur_len + 1
